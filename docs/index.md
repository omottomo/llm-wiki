...
└── tasks/                          # one folder per code-mode phase: plan.md (+ prd.json once authored)
    ...
    └── phase-19-mermaid-render-check/
        ├── plan.md                 # CI gate: parse every Mermaid block with the pinned mermaid (jsdom, no Chromium) — syntax only, not layout
        └── prd.json                # 4 tasks (3 builder + 1 manual)
```
...
| [tasks/phase-19-mermaid-render-check/](tasks/phase-19-mermaid-render-check/plan.md) | `plan.md` + `prd.json` — 4 tasks (3 builder + 1 manual), 2026-09-22: phase-18 shipped the diagram form but nothing ever parses a diagram — `check_diagrams` validates caption, citation, type, count and page type, and `lint_wiki.py:443` says outright that Mermaid syntax is not parsed because Python has no parser, naming a CI step as the remedy. Since the block renders in the reader's browser, a syntax error ships silently and surfaces as a red error box on the live site, with only a human PR preview in between. `scripts/check_mermaid.mjs` parses every fence with `mermaid.parse` under jsdom and runs in `verify.yml`'s `verify` job (Node already set up there; the `lint` job stays Node-free). `@mermaid-js/mermaid-cli` was rejected — it peer-depends on puppeteer, so each run would install a ~150 MB Chromium to answer a grammar question. The workflow reads the version straight out of `site/build.py`'s `MERMAID_VERSION` and the script re-asserts the match, so the checker can never drift from the parser the browser loads. **Syntax only** — overlapping labels, overflowing text and unreadable layout stay a human job on the PR |
