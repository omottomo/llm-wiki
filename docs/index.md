...
└── tasks/                          # one folder per code-mode phase: plan.md (+ prd.json once authored)
    ...
    ├── phase-16-iam-user-migration/
    │   └── implementation.md       # human-run runbook: move the local AWS profile from IAM user llm-wiki to devops, then delete llm-wiki
    ├── phase-17-infra-ci/
    │   └── plan.md                 # finish phase-13's unbuilt half: apply the read-only plan role, add infra.yml, make its check required
    ├── phase-18-wiki-diagrams/
    │   ├── plan.md                 # design spec: Mermaid diagrams on concept/entity pages — content rules, browser render, lint, ingest + wiki-illustrate skills
    │   └── implementation.md       # 8-task TDD plan (rules → lint → build → css → verify → skills → pilot → close-out)
    └── phase-20-moc-hubs/
        └── plan.md                 # index.md becomes an unpublished gateway: five category pages under wiki/categories/, category-aware lint + grouped/sortable listings, skills updated
```
...
| [architecture.svg](architecture.svg) | The architecture diagram embedded at the top of the root `README.md`: visitor band on top, then the three panels (local human + agent, GitHub CI, AWS). Hand-authored SVG — edit the file directly and re-render to check (`"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless --screenshot=out.png --window-size=1800,1036 docs/architecture.svg`). The three panels sit inside one `<g transform="translate(0,96)">`, so the band height is the only global offset. |
| [images/](images/) | README screenshots — `home.png`, `page.png`, `search.png`. Regenerated by building the site (`python3 site/build.py && npx -y pagefind@1 --site site/dist`), serving `site/dist`, and driving headless Chrome at 1440px / DPR 2. The search shot needs a real browser: Pagefind's wasm never resolves under `--virtual-time-budget`, so type into `#search input` and wait for `.pagefind-ui__result`. |
...
| [tasks/phase-16-iam-user-migration/](tasks/phase-16-iam-user-migration/implementation.md) | `implementation.md` only — 2026-09-03, **human-executed** (the agent wrote the runbook and answers questions; no `plan.md`, no `prd.json`): swap the local Terraform/CLI principal from IAM user `llm-wiki` to `devops` (same AWS account, both in `AdministratorAccessGroup`) and delete `llm-wiki` outright. Nothing pins the old user — CI assumes the `llm-wiki-deploy` OIDC **role**, the site bucket policy names only the CloudFront service, and Terraform state records no caller — so the whole migration is a credential swap whose proof is `terraform plan` printing `No changes.` under the new profile. Deliberately Korean (user executes it personally); no account IDs or key IDs in the file, per the phase-15 scrub. The `llm-wiki-*` bucket and role names stay — those are the project name, not the user. **Done 2026-09-03** — the runbook carries an execution record with the two residual risks (the second, unlocated access key was deleted without ever being traced; the `2 to add` in `plan` is phase-13's unapplied `llm-wiki-plan` role, not this migration) |
| [tasks/phase-17-infra-ci/](tasks/phase-17-infra-ci/plan.md) | `plan.md` only — 2026-09-03, **not started**: phase-13 designed a read-only OIDC role so `terraform plan` could run on pull requests, then shipped neither the `apply` nor the workflow — T03–T06 never ran, which is why `terraform plan` still reports `2 to add` (`aws_iam_role.plan`, `aws_iam_role_policy.plan`) and why nothing verifies `infra/` at all. Five tasks: apply the role (plus `cloudfront:Describe*`, missing from the `Get*`/`List*` prefixes and needed to refresh the CloudFront Function), add `.github/workflows/infra.yml` as one always-reporting `infra` job (fmt + `validate` unconditional, credentials and `plan` behind a `gh pr view --json files` probe — unfiltered because a required check plus a `paths:` filter deadlocks merges), add `infra` to ruleset `20497822`, prove both the green and the red path on a real PR, then record the rules. Two phase-13 premises expired and are corrected here: the repo is public with a working ruleset (so the check is **required**, not advisory — phase-13 could only make a broken `.tf` visible), and Actions inputs are secrets rather than variables. Also folds in the Cloudflare Pages → S3 + CloudFront correction that phase-9 left pending in §2 of `rules/site-code.md` |
| [tasks/phase-20-moc-hubs/](tasks/phase-20-moc-hubs/plan.md) | `plan.md` only — 2026-09-22, **done**: the 42-page virtualization/git/jev ingest pushed every index category past the ~30-entry scaling trigger in `rules/wiki-content.md` §2, leaving one 131-line flat list mixing five subjects — and `index.md` is the first file every skill reads. Five category pages under `wiki/categories/` now hold the per-page lines (AI coding 65 · infra 25 · virtualization 16 · git 16 · jev 10) and `index.md` is a five-line gateway that the site no longer publishes. `check_index_coverage` follows one hop (index → a category page it links → pages); `build_inbound_map` and `check_orphans` exclude `categories/` so the orphan check keeps its teeth; `site/build.py` groups every listing and tag page by category from the same links and adds a 제목순/최근 갱신순 sort toggle. Review changes the same day: "주제 관문" renamed to "카테고리" everywhere (folder, URL, copy), "여기서부터" → "처음이라면 이 순서로". `wiki-ingest` step 4.5 now updates the subject's category page and touches `index.md` only for a new subject — the same change landed in `wiki-query`/`wiki-lint`/`wiki-delete`/`wiki-refresh`/`wiki-illustrate`. Fixtures: golden-wiki proves the category path lints clean, `defects/category-not-linked-from-index` proves an unlinked category page covers nothing |
| [tasks/phase-18-wiki-diagrams/](tasks/phase-18-wiki-diagrams/plan.md) | `plan.md` + `implementation.md` — 2026-09-17 design spec and 8-task TDD plan, **T1–T8 done (2026-09-17)**: `concepts/` and `entities/` explain architectures, flows and hierarchies in prose and tables only, so the reader rebuilds the picture by hand. Decided in brainstorm: agent-drawn **Mermaid fenced blocks** in the page (no image files; GitHub previews render them), with a numbered cited caption, ≤3 per page, five allowed diagram types, `concepts/`·`entities/` only; third-party images only from licensed official docs after human approval, stored under `wiki/assets/`. Site: markdown-it fence hook → `<pre class="mermaid">`, pinned jsdelivr `mermaid@11.17.2` module loaded only on diagram pages, theme follows `data-theme` and re-renders on toggle, `assets/` copied to `dist/`. Lint: `check_diagrams` validates form (type, caption, citation, cap, scope) but never requires presence. Skills: `wiki-ingest` draws at write time; new `wiki-illustrate` backfills existing pages in a follow-up content-mode session. **No S3/CloudFront change** — `s3 sync` already covers `dist/`, no CSP/headers policy exists, Mermaid loads from the CDN. Pilot diagram on `concepts/kubernetes.md`; the remaining pages are backfilled by `wiki-illustrate` in a follow-up content-mode session |
