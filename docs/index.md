...
└── tasks/                          # one folder per code-mode phase: plan.md (+ prd.json once authored)
    ...
    └── phase-20-moc-hubs/
        └── plan.md                 # index.md becomes a gateway: five subject MOCs under wiki/moc/, MOC-aware lint coverage, skills updated
```
...
| [tasks/phase-20-moc-hubs/](tasks/phase-20-moc-hubs/plan.md) | `plan.md` only — 2026-09-22, **done**: the 42-page virtualization/git/jev ingest pushed every index category past the ~30-entry scaling trigger in `rules/wiki-content.md` §2, leaving one 131-line flat list mixing five subjects — and `index.md` is the first file every skill reads. Five MOC hubs under `wiki/moc/` now hold the per-page lines (AI coding 65 · infra 25 · virtualization 16 · git 16 · jev 10) and `index.md` is a gateway of five lines. `check_index_coverage` follows one hop (index → a MOC it links → pages); `build_inbound_map` and `check_orphans` exclude `moc/` so the orphan check keeps its teeth; `site/build.py` needed only a `SECTIONS` entry. `wiki-ingest` step 4.5 now updates the subject MOC and touches `index.md` only for a new subject — the same change landed in `wiki-query`/`wiki-lint`/`wiki-delete`/`wiki-refresh`/`wiki-illustrate`. Fixtures: golden-wiki proves the MOC path lints clean, `defects/moc-not-linked-from-index` proves an unlinked hub covers nothing |
