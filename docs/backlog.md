...
- ~~**색인이 임계치를 넘었다.**~~ → 2026-09-22 phase-20 에서 처리. 카테고리 페이지 5개(`wiki/categories/`)를 만들고 `index.md` 를 관문으로 바꿨다. 흡수 때 카테고리 페이지를 갱신하는 절차는 `wiki-ingest` §4-5 에 들어갔고, lint 가 카테고리 경유 등재를 인정한다. 원래 적어 둔 메모는 아래에 남긴다.
  - 원래 메모: **색인이 임계치를 넘었다.** `docs/rules/wiki-content.md` §2 의 확장 기준은 한 카테고리가 약 30개를 넘으면 MOC(Map of Content) 허브 페이지를 도입하라고 적는다. 2026-09-22 기준 Sources 56·Concepts 34·Entities 36 으로 세 카테고리가 모두 넘었다. 주제가 AI 코딩·IaC·가상화·Git·Jev 로 갈라졌으니 주제별 MOC 가 자연스러운 분할선이다 — 사람 결정 대기.
