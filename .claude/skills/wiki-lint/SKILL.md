...
Get the full page list from the category pages under `wiki/categories/` (`wiki/index.md` lists only the categories), then scan pages for:
...
> so the pass lands through a PR. *Why:* every skill run touches a category page under `wiki/categories/` and `docs/log.md`, so
