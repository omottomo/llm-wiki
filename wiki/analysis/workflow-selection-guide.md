...
이 글은 같은 AI에게 같은 일을 시키더라도 어떤 실행 방식을 고르느냐에 따라 결과와 비용이 크게 달라진다는 점을 다룬다. 그냥 대화로 시키는 것부터 작업 요령을 붙여 주기, 일을 나눠 맡기기, 스스로 절차를 짜게 하기까지를 하나의 스펙트럼 위에 놓고 비교한다. 오른쪽으로 갈수록 알아서 해 주는 정도가 커지지만 비용도 함께 커지므로, 결론은 작업 크기에 맞춰 필요한 만큼만 쓰라는 것이다.
