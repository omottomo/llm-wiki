...
MCP(모델 컨텍스트 프로토콜)는 AI 에이전트가 바깥 도구와 데이터를 쓸 수 있게 연결해 주는 공통 규격이다. 브라우저 조작, 문서 검색, 데이터베이스 조회처럼 AI가 혼자서는 할 수 없는 일을 이 연결을 통해 대신 시킬 수 있다. 다만 많이 연결할수록 좋은 것은 아니어서, 연결된 도구의 설명만으로도 AI가 읽어야 할 분량이 늘어 성능과 비용이 함께 나빠진다. "지금 하는 작업에 필요한 것만 켜 둔다"가 이 개념에서 가장 자주 반복되는 실전 교훈이다.
...
- 주방 비유: [[concepts/context-engineering|컨텍스트 엔지니어링]]이 좋은 재료를 골라주는 것이라면, MCP 설정은 수산시장·정육점·농장과 직통으로 연결하는 "외부 업체 어댑터"다 (→ [[sources/youtube-BssPGKsP60s|#5 조용히 설계한다]]).
- 데이터베이스 MCP를 연결하면 AI가 DB를 정해진 방식으로만 다루게 된다 — 도구 사용에 일종의 경계를 부여한다 (→ [[sources/youtube-DrekqeDlO1w|#14 하네스 문서 100번]] 참조). [[concepts/hooks|훅]]의 '도구 경계' 참조.
...
- 직관과 달리 도구는 적을수록 좋다. 도구가 많으면 에이전트가 "어떤 도구를 쓸지" 고민하는 데 에너지를 써서 정작 할 일에 집중을 못 한다(넷플릭스에서 뭐 볼지 30분 고민하다 결국 유튜브 켜는 것에 비유) (→ [[sources/youtube-6MYZ7fMhKPY|#21 바이브에서 에이전틱으로]]).
...
- 도구 설명이 큰 MCP는 그대로 쓰지 말고, **자주 쓰는 기능만 골라 래핑한 커스텀 MCP**를 만들어 쓴다. 필요한 엔드포인트만 래핑하면 토큰 절약은 물론 응답 품질도 올라간다 (→ [[sources/youtube-DCsv0rKKrN4|#7 메타 엔지니어 실전편]]).
...
- 단, "스킬·MCP를 다 설치했더니 Claude Code가 이상해졌다면 다 지우고 지금 작업에 필요한 것만 남기라"는 게 하네스 다이어트의 실천이다 (→ [[sources/youtube-6MYZ7fMhKPY|#21 바이브에서 에이전틱으로]]). [[concepts/harness-engineering|하네스 엔지니어링]]의 긴장② 참조.

## 연결
- [[concepts/harness-engineering|하네스 엔지니어링]], [[concepts/skills|스킬]], [[concepts/hooks|훅]], [[concepts/context-engineering|컨텍스트 엔지니어링]], [[concepts/claude-md|CLAUDE.md]] (둘 다 사용자 접근 가능 컨텍스트지만 MCP·스킬 메타데이터는 자동 주입 영역), [[entities/claude-code|Claude Code]].
