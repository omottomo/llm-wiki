...
```mermaid
flowchart TB
  subgraph SUB[서브에이전트 — 메인 세션 안의 위임]
    M1[메인 세션] --> S1[서브에이전트]
    S1 -- 결과만 반환 --> M1
  end
  subgraph TEAM[에이전트 팀 — 독립 인스턴스 여러 개]
    LEAD[메인 · 팀 리드] --> TL[(태스크 리스트)]
    LEAD -- 생성 · 작업 분배 --> T1[팀메이트]
    LEAD -- 생성 · 작업 분배 --> T2[팀메이트]
    T1 <-- 실시간 메시지 --> T2
    T1 --- TL
    T2 --- TL
  end
```
*그림 1. 두 구조의 차이 — 서브에이전트는 결과만 메인에 돌려주고, 에이전트 팀은 팀 리드가 태스크 리스트를 만들어 팀메이트에게 뿌리며 팀메이트끼리 직접 메시지를 주고받고 공유 작업 목록을 본다 (→ [[sources/youtube-Gb2VMWrUmZ0|#3 Claude Code Teams]])*
