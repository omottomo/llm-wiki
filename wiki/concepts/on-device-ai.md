...
온디바이스 AI는 인터넷 너머의 서버를 부르지 않고 휴대폰이나 노트북 같은 기기 안에서 직접 AI 모델을 돌리는 방식이다. 호출할 때마다 나가는 사용료가 들지 않고 데이터가 기기 밖으로 나가지 않는다는 점이 가장 큰 장점이다. 대신 기기에 올라갈 만큼 모델이 작아야 해서 성능에 한계가 있고, 그 한계를 어디까지 좁혔는지가 이 분야를 지켜보는 관전 포인트다. 이 위키에서는 주로 구글 Gemma 계열 모델을 통해 다룬다.
