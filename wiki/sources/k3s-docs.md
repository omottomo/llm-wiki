...
- **업그레이드**: 설치 스크립트 재실행(수동)과 Rancher의 system-upgrade-controller를 쓴 선언형 자동 업그레이드(`Plan` CRD) 두 방식이다. 서버 먼저, 그다음 에이전트라는 순서는 어느 쪽에서든 지켜야 한다.
...
- **포트** — 6443(apiserver·supervisor)은 모든 노드가 서버에 닿아야 한다. Flannel VXLAN이면 UDP 8472, WireGuard면 UDP 51820(IPv6는 51821), 메트릭 서버를 쓰면 10250, 내장 etcd HA면 서버끼리 2379·2380이 필요하다. "VXLAN 포트를 외부에 노출하면 클러스터 네트워크가 누구에게나 열린다"고 경고한다.
