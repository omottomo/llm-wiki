...
## 기존 위키와의 연결
- 강화: [[concepts/llm-basics|LLM 기초]]의 트랜스포머/어텐션/사전훈련/RLHF 설명을 가장 기초 수준에서 뒷받침하는 토대 자료. 재생목록 전체가 다루는 "에이전트가 컨텍스트를 어떻게 처리하는가"의 출발점이 되는 배경 지식.
- 모순: 없음 (개념 입문 자료라 다른 소스와 충돌하지 않음).
- 신규: [[concepts/llm-basics|LLM 기초]] (확률 분포 예측·역전파·트랜스포머·어텐션·피드포워드·RLHF·해석 불가능성). 인물/조직으로 [[entities/openai|OpenAI]](GPT-3, ChatGPT 맥락)와 구글의 트랜스포머 연구 언급.
