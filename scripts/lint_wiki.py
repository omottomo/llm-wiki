...
여전히 LLM(에이전트)의 몫이고, 이 스크립트는 다음만 검사한다:
  ...
  3. frontmatter 필수 키 (title/type/created/updated/sources/tags) + type 값 +
     ...
     aliases 값의 리스트 형식
  4. 색인 등재 여부 — 모든 페이지가 index.md 또는 index.md 가 링크한 MOC 허브에 올라 있는가
  5. 고아 페이지 — index.md·MOC 허브 외에 아무 페이지도 링크하지 않는 페이지
...
# phase-20 — 주제 관문 허브. index.md 는 관문만 링크하고 페이지별 줄은 여기 있다.
MOC_PREFIX = "moc/"
...
def check_index_coverage(pages):
    """index.md 또는 index.md 가 링크한 MOC 허브에 모든 페이지가 등재돼 있는가 (phase-20).

    index.md 는 관문 목록이고 페이지별 줄은 wiki/moc/*.md 가 들고 있다. 그래서 등재 판정은
    'index.md 가 링크한 MOC' 까지 한 단계만 따라간다 — index 가 링크하지 않는 MOC 에만 적힌
    ...
    for moc_key in sorted(k for k in index_targets if k.startswith(MOC_PREFIX)):
        moc_path = WIKI / f"{moc_key}.md"
        if moc_path.exists():
            listed |= {normalize_target(t) for t in WIKILINK_RE.findall(moc_path.read_text(encoding="utf-8"))}
    for page in pages:
        key = page_key(page)
        if key in ("index", "overview"):
            continue
        if key not in listed:
            add("색인", f"wiki/{key}.md 가 index.md·MOC 어디에도 등재되어 있지 않음")


def build_inbound_map(pages) -> dict:
    """페이지별 인바운드 링크 소스 페이지 집합. index.md 와 MOC 허브는 소스로 집계하지 않는다
    ...
    for page in pages:
        src = page_key(page)
        if src == "index" or src.startswith(MOC_PREFIX):
            continue
        text = page.read_text(encoding="utf-8")
...
def check_orphans(pages):
    ...
    for key, sources in sorted(inbound.items()):
        if key in ("index", "overview") or key.startswith(MOC_PREFIX):
            continue
        if not sources:
